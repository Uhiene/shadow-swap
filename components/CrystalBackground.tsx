...
export default function CrystalBackground() {
  ...
  /* Right ➜ Left */
  @keyframes swimLeft {
    0%   { transform: translateX(110vw) translateY(0px) scaleX(var(--sx)); }
    15%  { transform: translateX(80vw)  translateY(-18px) scaleX(var(--sx)); }
    30%  { transform: translateX(65vw)  translateY(10px) scaleX(var(--sx)); }
    45%  { transform: translateX(50vw)  translateY(-14px) scaleX(var(--sx)); }
    60%  { transform: translateX(30vw)  translateY(8px) scaleX(var(--sx)); }
    75%  { transform: translateX(15vw)  translateY(-12px) scaleX(var(--sx)); }
    100% { transform: translateX(-200px) translateY(0px) scaleX(var(--sx)); }
  }
`}</style>
          ...
          style={
            {
              ...
              height: "auto",
              opacity: s.opacity,
              animation: `${s.flip ? "swimRight" : "swimLeft"} ${
                s.duration
              }s linear infinite`,
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 60% 40% at 20% 100%, rgba(217,70,239,0.08) 0%, transparent 70%)",
        }}
      />
    </div>
  );
